...
class ListViewController: UIViewController, UITableViewDelegate, UITableViewDataSource, SwipeTableViewCellDelegate, SKPhotoBrowserDelegate, UIViewControllerPreviewingDelegate {
    ...
    func previewingContext(_ previewingContext: UIViewControllerPreviewing, viewControllerForLocation location: CGPoint) -> UIViewController? {
        ...
        detailVC.mainStatus.append(StoreStruct.currentList[indexPath.row])
...
//        self.navigationController?.navigationBar.barTintColor = Colours.tabUnselected
    ...
    func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
        return StoreStruct.currentList.count
    }
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if StoreStruct.currentList.count == 0 {
            ...
            if StoreStruct.currentList[indexPath.row].reblog?.mediaAttachments.isEmpty ?? StoreStruct.currentList[indexPath.row].mediaAttachments.isEmpty || (UserDefaults.standard.object(forKey: "sensitiveToggle") != nil) && (UserDefaults.standard.object(forKey: "sensitiveToggle") as? Int == 1) {
                ...
                cell.delegate = self
                cell.backgroundColor = Colours.white
                cell.configure(StoreStruct.currentList[indexPath.row])
    ...
    @objc func didTouchProfile(sender: UIButton) {
        ...
        let controller = ThirdViewController()
        if StoreStruct.currentList[sender.tag].reblog?.account.username ?? StoreStruct.currentList[sender.tag].account.username == StoreStruct.currentUser.username {} else {
            controller.fromOtherUser = true
        }
        if self.currentIndex == 0 {
            controller.userIDtoUse = StoreStruct.currentList[sender.tag].reblog?.account.id ?? StoreStruct.currentList[sender.tag].account.id
            self.navigationController?.pushViewController(controller, animated: true)
        } else if self.currentIndex == 1 {
            controller.userIDtoUse = StoreStruct.statusesLocal[sender.tag].reblog?.account.id ?? StoreStruct.statusesLocal[sender.tag].account.id
            self.navigationController?.pushViewController(controller, animated: true)
        } else {
            controller.userIDtoUse = StoreStruct.statusesFederated[sender.tag].reblog?.account.id ?? StoreStruct.statusesFederated[sender.tag].account.id
            self.navigationController?.pushViewController(controller, animated: true)
        }
    }
                    ...
                    StoreStruct.client.run(request2) { (statuses) in
                        DispatchQueue.main.async {
                            if let cell = tableView.cellForRow(at: indexPath) as? MainFeedCell {
                                ...
                                let cell = tableView.cellForRow(at: indexPath) as! MainFeedCellImage
                                if sto[indexPath.row].reblog?.reblogged! ?? sto[indexPath.row].reblogged! || StoreStruct.allBoosts.contains(sto[indexPath.row].id) {
                                    cell.moreImage.image = nil
                                    cell.moreImage.image = UIImage(named: "boost")
                                } else {
                                    cell.moreImage.image = nil
