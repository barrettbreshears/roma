...
class PadDetailViewController: UIViewController, UITableViewDelegate, UITableViewDataSource, SwipeTableViewCellDelegate, SKPhotoBrowserDelegate, UIViewControllerPreviewingDelegate {
    ...
    @objc func didTouchLike(sender: UIButton) {
        ...
        if self.mainStatus[0].reblog?.favourited! ?? self.mainStatus[0].favourited! || StoreStruct.allLikes.contains(self.mainStatus[0].reblog?.id ?? self.mainStatus[0].id) {
    ...
    @objc func didTouchBoost(sender: UIButton) {
        ...
        if self.mainStatus[0].reblog?.reblogged! ?? self.mainStatus[0].reblogged! || StoreStruct.allBoosts.contains(self.mainStatus[0].reblog?.id ?? self.mainStatus[0].id) {
            if (UserDefaults.standard.object(forKey: "hapticToggle") == nil) || (UserDefaults.standard.object(forKey: "hapticToggle") as! Int == 0) {
                let notification = UINotificationFeedbackGenerator()
                notification.notificationOccurred(.success)
            }
...
//                            vc.previewFont = UIFont.systemFont(ofSize: 14)
            ...
            Alertift.actionSheet(title: nil, message: nil)
                ...
                .action(.default("Translate".localized), image: UIImage(named: "translate")) { (action, ind) in
                    ...
                    var bodyText = self.mainStatus[0].reblog?.content.stripHTML() ?? self.mainStatus[0].content.stripHTML()
...
//                                    vc.previewFont = UIFont.systemFont(ofSize: 14)
                    ...
                    Alertift.actionSheet(title: nil, message: nil)
                        ...
                        .action(.default("Translate".localized), image: UIImage(named: "translate")) { (action, ind) in
                            ...
                            let allowed = NSMutableCharacterSet.alphanumeric()
                            allowed.addCharacters(in: unreserved)
                            var bodyText = sto[indexPath.row].content.stripHTML()
...
//                                    vc.previewFont = UIFont.systemFont(ofSize: 14)
    ...
    func loadLoadLoad() {
        ...
        self.splitViewController?.view.backgroundColor = Colours.grayDark3
